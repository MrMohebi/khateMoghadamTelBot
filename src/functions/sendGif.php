<?php
function sendGif($bot,$chatId, $text){
    if (preg_match("/\b"."هعی"."\b/u", $text)){
        $bot->sendAnimation($chatId,"CgACAgQAAx0CTwcRQAADLGDVqJP5p0fOriGNHt504mxlSMg6AAIXCgACsA6BUM1SJhMGGCXdIAQ");
        ...
        $bot->sendAnimation($chatId,$gay_list[array_rand($gay_list)]);
    }elseif (preg_match("/\b"."مرام|معرفت|مرامی|معرفتی"."\b/u", $text)){
        $bot->sendAnimation($chatId,"CgACAgQAAx0CTwcRQAADQGDVsZQvqj5amOUaTARF5ZOCQry6AAJ0CgACMdG4UZDkRTQUCrOfIAQ");
        ...
        $bot->sendAnimation($chatId,$mame_list[array_rand($mame_list)]);
    }elseif (preg_match("/\b"."سیگار|گنگ|گنگه|گنگا"."\b/u", $text)){
        $bot->sendAnimation($chatId,"CgACAgQAAx0CTwcRQAADbWDVtag_MvNeYBD0wWFlX1EOsbh4AAIrCQACrAkhUMahkBF1ep6tIAQ");
    }
}